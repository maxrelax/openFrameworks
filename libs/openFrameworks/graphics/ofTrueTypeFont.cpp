...
bool ofTrueTypeFont::load(string _filename, int _fontSize, bool _bAntiAliased, bool _bFullCharacterSet, bool _makeContours, float _simplifyAmt, int _dpi) {
	...
	for (int i = 0 ; i < nCharacters; i++){
		...
		expanded_data[i].allocate(width, height, OF_PIXELS_MONO);
		//-------------------------------- clear data:
		...
		if (bAntiAliased == true){
			...
			bitmapPixels.setFromExternalPixels(bitmap.buffer,bitmap.width,bitmap.rows,OF_PIXELS_MONO);
	...
	atlasPixelsLuminanceAlpha.allocate(w,h,OF_PIXELS_MONO);
...
void ofTrueTypeFont::drawString(string c, float x, float y) const{
	...
	ofGetCurrentRenderer()->drawString(c,x,y,1.0, OF_BITMAPMODE_SCREEN);
